# audit-determinism-repro

Adversarial auditor for determinism and reproducibility: unseeded RNG in reported numbers, float
non-deterministic reduces, thread-order sensitivity, provenance stamping (`code_commit`/`schema_version`),
and reproducibility-hash discipline. Use on any code whose output feeds a ranking, a verdict, or a persisted
artifact. This is a read-only audit role: use repository reads, search, and shell probes, but do not edit files.
