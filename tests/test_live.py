...
def _build(tmp_path, *, cfg=None, t0_ms=10_000, prepare=None, **over):
    ...
    return LiveFollowSystem.build(
        ...
        poll_interval_s=0.0, prepare=prepare, **over)
...
def test_reroll_registers_new_manifest_and_adopts(tmp_path):
    calls = {"prepare": 0}
    async def prepare(t0): calls["prepare"] += 1
    sys = _build(tmp_path, prepare=prepare)
    rid1 = sys.run_id
