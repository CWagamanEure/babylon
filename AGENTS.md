# Babylon - Codex working instructions
...
2. **Positive-control MDE <= the effect size we care about.** The pipeline must be *shown* able to recover
   ...
   **BUT "inconclusive" is not a permanent shield - the anti-ratchet rule.** If a design is structurally
...
4. **Construction-conservatism check.** Confirm the null band / test was not accidentally built *against*
   the finding (e.g. a null band aggregated at the wrong level and 3.5x too wide manufactures a false null).
...
**The mirror failure - OVER-CARRY - actually happened (2026-07-04) and this rule now guards it explicitly.**
...
- **Reusable audit subagents** are defined by the role manuals in `agents/audit-*.md` (correctness,
  stats-rigor, firewall-leakage, data-integrity, determinism-repro, docs-consistency) and registered for
  native Codex discovery by `.codex/agents/audit-*.toml`. They all follow `audit/AUDIT_PROTOCOL.md`
  (resource safety, verify-before-report, blast-radius severity, no edits). Invoke the matching named agent
  for a swarm audit and include a run-specific scope note like `audit/00_GROUND_RULES.md` in the task.
...
oracle in RESTORE_PLAN section "MONTH GATE RESULT — 2025-08" (87.9M fills, zhash 18.2%, the transition counts).

`hl_hist/*.csv.lz4` is per-coin **asset-context metadata** (funding/OI/px), NOT the fills tape.
